...
class GoogleNews:
    ...
    def __init__(self,lang="en",period="",start="",end="",encode="utf-8",region=None, proxies=None):
        self.__texts = []
        self.__links = []
        self.__results = []
        self.__totalcount = 0
    ...
    def build_response(self):
        url = self.url.replace(
            "search?",
            f"search?hl={self.__lang}&gl={self.__lang}&"
        )

        response = requests.get(url, headers=self.headers, timeout=15, proxies=self.proxies)
        ...
        if stats and isinstance(stats, ResultSet):
            m = re.search(r'[\d,]+', stats[0].get_text())
            if m:
                self.__totalcount = int(m.group().replace(',', ''))
            else:
    ...
    def get_page(self, page=1):
        ...
        try:
            if self.__start != "" and self.__end != "":
                self.url = "https://www.google.com/search?q={}&lr=lang_{}&biw=1920&tbm=nws&bih=976&source=lnt&&tbs=lr:lang_1{},cdr:1,cd_min:{},cd_max:{},sbd:1&tbm=nws&start={}".format(self.__key,self.__lang,self.__lang,self.__start,self.__end,(10 * (page - 1)))
            elif self.__period != "":
                self.url = "https://www.google.com/search?q={}&lr=lang_{}&biw=1920&tbm=nws&bih=976&source=lnt&&tbs=lr:lang_1{},qdr:{},,sbd:1&tbm=nws&start={}".format(self.__key,self.__lang,self.__lang,self.__period,(10 * (page - 1)))
            else:
                self.url = "https://www.google.com/search?q={}&lr=lang_{}&biw=1920&tbm=nws&bih=976&source=lnt&&tbs=lr:lang_1{},sbd:1&tbm=nws&start={}".format(self.__key,self.__lang,self.__lang,(10 * (page - 1)))
        except AttributeError:
            raise AttributeError("You need to run a search() before using get_page().")
        # try:
        result = self.build_response()
    ...
    def get_news(self, key="",deamplify=False):
        ...
        response = requests.get(self.url, headers=self.headers, timeout=15, proxies=self.proxies)
        response.raise_for_status()

        content = Soup(response.text, "html.parser")
        tags = [
            el for el in content.select('c-wiz[data-node-index*="1;"]')
            if len(el.attrs.get('class', [])) == 3  # ровно 3 класса
        ]


        # with open("index.html", "w", encoding="utf-8") as f:f.write(content.prettify())

        for article in tags:
            title = article.select_one("a[aria-label]").text
            jslog = article.select_one('a[aria-hidden="true"]').attrs.get("jslog")

            temp = jslog.split(";")
            temp2 = temp[1].split(":")[-1]
            decoded = base64.b64decode(temp2).decode("utf-8")
            data = json.loads(decoded)
            url = data[-1]

            datetime_chars = article.find('time').get('datetime')
            datetime_obj = parse(datetime_chars).replace(tzinfo=None)
            self.__results.append({'title': title, 'datetime': datetime_obj, 'url': url})
